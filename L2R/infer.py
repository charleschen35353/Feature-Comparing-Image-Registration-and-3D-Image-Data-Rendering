...
ckpt = tf.train.Checkpoint(step=tf.Variable(1, dtype=tf.int64), optimizer=tf.keras.optimizers.Adam(1e-4), net=net)
ckpt.restore(tf.train.latest_checkpoint('./L2R/'))
print("model_restored")
ip1, ip2, ip3, ip4, ip5 = 
homo = net(dl.load_data()[0])
aligned = np.convolve(ip5, homo, padding = 'SAME')
