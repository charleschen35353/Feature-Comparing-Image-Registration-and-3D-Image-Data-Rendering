...
def calc_homographies(kprs, kpss, matches):
    ...
    for i in range(len(matches)):
        ...
        if len(matches[i]) > 7:
            for j in range(len(matches[i])):
                ...
                if kpss[i][matches[i][j].queryIdx] is not None:
                    p1[j, :] = kpss[i][matches[i][j].queryIdx].pt  
                else:
                    p1[j, :] = (DUMMY_COOR_MAX, DUMMY_COOR_MAX)

                if kprs[i][matches[i][j].trainIdx] is not None:
                    p2[j, :] = kprs[i][matches[i][j].trainIdx].pt 
                else:
                    p2[j, :] = (DUMMY_COOR_MIN, DUMMY_COOR_MIN)
